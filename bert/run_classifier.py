...
class MergeProcessor(DataProcessor):
  ...
  def get_test_examples(self, data_dir):
    if FLAGS.use_val_test:
        ...
        dataset_split = 'test'
    """See base class."""
    json_lines = self._read_lines(os.path.join(os.path.dirname(os.path.dirname(data_dir)), 'article_embeddings', 'output_article', dataset_split + '.jsonl')) if FLAGS.artemb else None
    return self._create_examples(
...
def main(unused_argv):
  ...
  if FLAGS.do_predict:
    ...
    predict_file = os.path.join(os.path.dirname(FLAGS.output_dir), 'tfrecords', predict_name + ".tf_record")
