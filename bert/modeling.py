  ...
  def __init__(self,
    ...
    with tf.variable_scope(scope, default_name="bert"):
      ...
      with tf.variable_scope("encoder"):
        ...
        self.all_encoder_layers, to_return = transformer_model(
            input_tensor=self.embedding_output,
        ...
        self.pre_coref_attention_scores, self.attention_scores = to_return
...
def attention_layer(from_tensor,
  ...
  pre_coref_attention_scores = attention_scores
  if coref_attentions is not None:
      attention_scores = tf.transpose(attention_scores, [1, 0, 2, 3])
      attention_scores_list = tf.unstack(attention_scores)
      attention_scores_list[coref_head] = coref_attentions
      attention_scores = tf.stack(attention_scores_list)
      # coref_head_t = tf.constant([[coref_head]])
      # attention_scores = tf.scatter_nd_update(attention_scores, coref_head_t, coref_attentions)
      attention_scores = tf.transpose(attention_scores, [1, 0, 2, 3])
  ...
  return context_layer, pre_coref_attention_scores, attention_scores
...
def transformer_model(input_tensor,
  ...
  for layer_idx in range(num_hidden_layers):
    if layer_idx == coref_layer:
      my_coref_attentions = coref_attentions
      my_coref_head = coref_head
    else:
      my_coref_attentions = None
      my_coref_head = None
    with tf.variable_scope("layer_%d" % layer_idx):
      ...
      with tf.variable_scope("attention"):
        ...
        with tf.variable_scope("self"):
          attention_head, pre_coref_attention_scores, attention_scores = attention_layer(
              ...
              coref_attentions=my_coref_attentions,
              coref_head=my_coref_head)
          attention_heads.append(attention_head)
          if layer_idx == coref_layer:
              to_return = (pre_coref_attention_scores, attention_scores)
  ...
  if do_return_all_layers:
    ...
    return final_outputs, to_return
  else:
