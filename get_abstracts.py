...
out_dir = os.path.expanduser('~') + '/data/cnn_dailymail_logan'
...
def read_text_file(text_file, encode=False):
  lines = []
  with open(text_file, "r") as f:
    for line in f:
      if encode:
        lines.append(line.encode('utf-8').strip())
      else:
...
def get_art_abs(story_file):
  ...
  abstract = [sent for sent in highlights]
...
def write_to_bin(url_file, out_dir, dataset_split):
  ...
  url_list = read_text_file(url_file, encode=True)
  ...
  out_file = os.path.join(out_dir, 'abstracts_' + dataset_split + '.tsv')
