...
# tf.boolean_mask
    ...
    def _add_seq2seq(self):
        ...
        with tf.variable_scope('seq2seq'):
            ...
            with tf.variable_scope('decoder'):
                decoder_outputs, self._dec_out_state, self.attn_dists, self.p_gens, self.coverage, self.pre_attn_dists = self._add_decoder(emb_dec_inputs)
    ...
    def build_graph(self):
        ...
        t1 = time.time()
        logging.info('Time to build graph: %i seconds', t1 - t0)
    ...
    def run_embedder(self, sess, token_inputs):
        feed_dict = {self.token_inputs: token_inputs}
        to_return = self.token_embeddings
    ...
    def decode_onestep(self, sess, batch, latest_tokens, enc_states, dec_init_states, prev_coverage, mmr_score):
        ...
        assert len(results['pre_attn_dists'])==1
        pre_attn_dists = results['pre_attn_dists'][0].tolist()
        ...
        return results['ids'], results['probs'], new_states, attn_dists, p_gens, new_coverage, pre_attn_dists
