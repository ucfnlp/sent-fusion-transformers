                ...
                for word_idx in word_indices_list:
                    if word_idx + to_add >= len(imp_mask):
                        if len(imp_mask) == hps.max_enc_steps:
                            continue
                        elif not self.hps.tag_tokens:
                            continue
                        else:
                            print ('Warning: word_idx + to_add (%d) is larger than imp_mask size (%d)' % (word_idx + to_add, len(imp_mask)))
                            continue
        ...
        while True:
            ...
            if self._hps.by_instance:   # if we are running iteratively on only instances (a singleton/pair + a summary sentence), not the whole article
                for abs_idx, abstract_sentence in enumerate(abstract_sentences):
                    ...
                    if article_lcs_paths_list is None:
                        inst_article_lcs_paths_list = None
                    else:
    ...
    def text_generator(self, example_generator):
        ...
        while True:
            ...
            try:
                ...
                if self._hps.coref_dataset:
                    names_to_types = [('raw_article_sents', 'string_list'),
                                      ('similar_source_indices', 'delimited_list_of_tuples'),
                ...
                if self._hps.coref_dataset:
                    article_lcs_paths_list = [[list(range(len(raw_article_sents[sent_idx]))) for sent_idx in source_indices] for source_indices in ssi]
